...
# Inputs : take sentence, search_word, replace_word
...
# replace blank with answer if the prompted user_answer is equal to the answer
    ...
    while index < len(answer):
        # initialize guess_left and min_guesses
        guess_left = 2
        min_guesses = 0
...
# Input : user_input of the difficulty level
# select difficulty of the quiz among 0,1,2
# if user_inputs are not 0 or 1 or 2, make users to choose again
    print """
    ###################################
...
# input : user_input
# take user input and use blank_quiz function to provide question and answer to users
    if user_input == "0" :
...
def run_quiz() :
# run quiz using the previously created funnctions - 1)select_difficulty, 2)set_answer
